...
In many software companies the percentage of AI generated code is increasing at a fast pace. The code churn is accelerating due to both developer-led features and fully automated 'AI cleanups and codemods'. Many developers feel much more productive and their number of landed changes has gone up significantly. The shipped code seems to work ok, if not the AI agent can root cause and fix bugs quickly.
...
And trusted code is not the same as used code. Used code has survived contact with reality. It has been lived with. Its rough edges are less theoretical. Its behavior is less mysterious. That kind of code carries operational memory in a way that fresh output does not.
